...
export default function GearMenu(shop) {
  const { gear, weapons, showDetails, gearDetails, slideDetails } = useSelector(state => state.fetcherSlice);
        ...
        shop.type.map((item, i) => {
          ...
          if (item.category && shop.filter && item.category.includes(shop.filter)) {
            ...
            if (item.shopData?.category === 'Armor') {
              return <div key={i} className={`w-full h-full flex justify-center items-center flex-col  ${style}`}>
